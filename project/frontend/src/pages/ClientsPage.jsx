...
function ClientsPage() {
  ...
  // Optionally load existing clients from the backend on component mount
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.get("http://localhost:8000/sanctum/csrf-cookie", { withCredentials: true })
      .then(() => {
        axios
          .post("http://localhost:8000/api/clients", newClient, { withCredentials: true })
          .then(response => {
          ...
          .catch(error => {
            console.error("Erreur adding client:", error);
          });
      })
      .catch(error => {
  ...
  const handleDelete = (id) => {
    ...
    modal.querySelector('.confirm-btn').addEventListener('click', () => {
      axios.delete(`http://localhost:8000/api/clients/${id}`, { withCredentials: true })
        .then(response => {
        ...
        .catch(error => {
          console.error('Erreur deleting client:', error);
          document.body.removeChild(modal);
        });
    });
  ...
  const handleEditSubmit = (e) => {
    ...
    axios.get("http://localhost:8000/sanctum/csrf-cookie", { withCredentials: true })
      .then(() => {
        axios.put(`http://localhost:8000/api/clients/${updatedClient.id}`, updatedClient, { withCredentials: true })
          .then(response => {
            setClients(clients.map(c => (c.id === updatedClient.id ? response.data : c)));
            setEditingClient(null);
            showNotification('Client mis à jour avec succès');
          })
          .catch(error => {
            console.error("Erreur updating client:", error);
          });
      })
      .catch(error => {
        console.error("Erreur fetching CSRF cookie:", error);
  ...
  return (
    <div id="add-product-page" >
      <h1 className="mb-4">
      <svg xmlns="http://www.w3.org/2000/svg" width="50" height="30" fill="currentColor" className="bi bi-people-fill" viewBox="0 0 16 16">

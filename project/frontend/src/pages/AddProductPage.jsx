...
function AddProductPage() {
  ...
  useEffect(() => {
    axios.get("http://localhost:8000/api/inventories")
      ...
      .catch((err) => console.error("Erreur fetching inventories:", err));
  }, [setProducts]);
  ...
  const handleSubmit = (e) => {
    ...
    axios.get("http://localhost:8000/sanctum/csrf-cookie", { withCredentials: true })
      .then(() => {
        axios
          .post("http://localhost:8000/api/inventories", newProduct, { withCredentials: true })
          .then(response => {
            setProducts([...products, response.data]);
            form.reset();
          })
          .catch(error => {
            console.error("Erreur adding product:", error);
          });
      })
      .catch(error => {
